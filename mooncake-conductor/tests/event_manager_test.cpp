...
using mooncake::conductor::common::PublisherKind;
using mooncake::conductor::common::ServiceConfig;
using mooncake::conductor::kvevent::EventManager;
using mooncake::conductor::kvevent::EventManagerTestPeer;
using mooncake::conductor::kvevent::KVEventHandler;
using mooncake::conductor::kvevent::KVEventHandlerTestPeer;
...
using mooncake::conductor::zmq::DecodedBatch;
using mooncake::conductor::zmq::MessageMetadata;
using mooncake::conductor::zmq::MooncakeClearedEvent;
using mooncake::conductor::zmq::MooncakeEvent;
...
HashProfile TestProfile(std::string python_hash_seed = "0",
    ...
    const mooncake::conductor::common::HashProfileConfig source{
        .strategy = "vllm_v1",
...
void ExpectRankMapsAligned(const Json::Value& instance) {
    ...
    EXPECT_EQ(ranks, instance["rank_matches"].getMemberNames());
...
Json::Value ServiceJson(const ServiceConfig& service) {
    ...
    value["type"] = std::string(
        mooncake::conductor::common::PublisherKindName(service.publisher_kind));
...
TEST(EventManager, StopWaitsForHttpServerShutdown) {
    auto manager = std::make_unique<EventManager>(
        std::vector<mooncake::conductor::common::ServiceConfig>{}, 0);
...
TEST_F(QueryHttpTest, ReturnsExactSharedCacheResponse) {
    ...
    EXPECT_EQ(body["instances"].getMemberNames(),
              (std::vector<std::string>{"1", "2"}));
    ASSERT_NO_FATAL_FAILURE(ExpectRankMapsAligned(body["instances"]["1"]));
...
TEST_F(QueryHttpTest, ReturnsOrderedFourBlockTierBoundaries) {
    ...
    const std::vector<int32_t> tokens = Sequence(1000, 64);
    const auto prefixes =
    ...
    EXPECT_EQ(body, expected);
    ASSERT_NO_FATAL_FAILURE(
        ExpectRankMapsAligned(body["instances"]["ordered"]));
}
...
TEST_F(QueryHttpTest, KeepsZeroHitRegisteredRanksInBothRankMaps) {
    ...
    for (const auto& [instance_id, rank] :
         std::vector<std::pair<std::string, std::string>>{{"1", "0"},
                                                          {"2", "1"}}) {
...
TEST_F(QueryHttpTest, AcceptsEmptyTokensAndSignedInt32Boundaries) {
    ...
    ASSERT_NO_FATAL_FAILURE(ExpectRankMapsAligned(empty["instances"]["2"]));
...
TEST_F(RegistrationHttpTest, PartialUnregisterKeepsInstanceUntilLastRank) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        ExpectRankMapsAligned(registered["instances"]["instance-1"]));
    ...
    ASSERT_NO_FATAL_FAILURE(
        ExpectRankMapsAligned(partial["instances"]["instance-1"]));
...
TEST(KVEventHandlerTest, VllmClearPreservesOtherEngineAndSharedOwners) {
    ...
    EXPECT_TRUE(DispatchVllm(handler_a, engine_a,
                             {mooncake::conductor::zmq::VllmClearedEvent{}}, 0)
                    .empty());
...
TEST(KVEventHandlerTest, MooncakeDiskOnlyHitUsesDiskBoundary) {
    EventManager manager({}, 0);
    const auto engine = VllmService("engine", "default", 0, 4);
    ASSERT_TRUE(
        manager.GetIndexer()->Register(RegistrationFor(engine)).error.empty());
    ...
    EXPECT_TRUE(
        DispatchMooncake(handler, pool,
                         {MooncakeStored(engine, prefix.value, "disk", "DISK")})
            .empty());
    EXPECT_EQ(KVEventHandlerTestPeer::BindingCount(handler), 1u);
    const auto result =
        manager.GetIndexer()->Query(ContextFor(engine), tokens).at("engine");
